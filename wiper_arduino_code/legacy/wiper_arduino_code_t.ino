...
void loop()
{
    ...
    if (power)
    {
        ...
        if (abs(TargetM1) < minimum_limit)
        {
            motor1.drive(0);
        }
        else
        {
            motor1.drive(TargetM1);
        }
        if (abs(TargetM2) < minimum_limit)
        {
            motor2.drive(0);
        }
        else
        {
            motor2.drive(TargetM2);
        }
    }
    else
    {
    ...
    float dt = (currTime - lastIMUTimestamp) / 1000.0;
...
void commandControl(bool isBluetooth)
{
    ...
    if (cmd.length() > 0)
    {
        ...
        double rpm1 = cmd.substring(0, cmd.indexOf(',')).toDouble();
        cmd.remove(0, cmd.indexOf(',') + 1);
        double rpm2 = cmd.substring(0, cmd.indexOf('|')).toDouble();
        cmd.remove(0, cmd.indexOf('|') + 1);
        //格式为 carx,cary|targetx,targety|mode|power|rpm1,rpm2|
...
void reportData(bool isBluetooth)
{
    ...
    // Send IMU data (imuX, imuY)
    dtostrf(imuX, numLen, numDec, tempBuffer);
    snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s ", tempBuffer);
    
    dtostrf(imuY, numLen, numDec, tempBuffer);
    snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s\n", tempBuffer);
